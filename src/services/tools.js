...
- OPTION CHAIN BOUNDARIES: After the first fetch, check if the returned strikes cover the user's target delta/strike range. If the edge strike's delta doesn't reach the target (e.g. user wants delta 0.15 but the furthest OTM strike has delta 0.40), STOP — do NOT fetch progressively higher/lower strikePriceNear trying to find it. The exchange simply doesn't list strikes that far OTM. Report the closest available strike and its delta, and explain that the target delta is beyond the listed chain.
